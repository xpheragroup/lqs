        ...
        if self.state in ('borrador compra'):
            ...
            if not self.requisicion_interna_purchase:
                for line in self.order_line:
                    last_price = self.env['purchase.order.line'].search([('product_id','=',line.product_id.id)])
                    if last_price:
                        last_price_id = 0
                        for purchase_line in last_price:
                            last_price_id_aux = purchase_line.id
                            last_price_warehouse = self.env['purchase.order'].search([('id','=',purchase_line.order_id.id)])
                            if last_price_warehouse:
                                if last_price_warehouse.picking_type_id == self.picking_type_id:
                                    if last_price_id < last_price_id_aux:
